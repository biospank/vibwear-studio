...
public class AudioClipRecorder {
    ...
    public AudioClipRecorder(AudioClipListener clipListener) {
        this.clipListener = clipListener; heard = false;
        task = null;
    }
    ...
    public boolean startRecording() {
        return startRecording(RECORDER_SAMPLERATE_8000, AudioFormat.ENCODING_PCM_16BIT);
    }

    /**
     * start recording: set the parameters that correspond to a buffer that * contains millisecondsPerAudioClip milliseconds of samples
     */
    public boolean startRecordingForTime(int millisecondsPerAudioClip, int sampleRate, int encoding) {
        ...
        int bufferSize = determineCalculatedBufferSize(sampleRate, encoding, numSamplesRequired);
        return doRecording(sampleRate, encoding, bufferSize, numSamplesRequired, DEFAULT_BUFFER_INCREASE_FACTOR);
    }

    /**
     * start recording: Use a minimum audio buffer and a read buffer of the same * size.
     */
    ...
    private int determineMinimumBufferSize(final int sampleRate, int encoding) {
        int minBufferSize = AudioRecord.getMinBufferSize(sampleRate,
                AudioFormat.CHANNEL_IN_MONO, encoding); return minBufferSize;
    }

    /**
     * Calculate audio buffer size such that it holds numSamplesInBuffer and is bigger than the minimum size<br>
     * @param numSamplesInBuffer
     * Make the audio buffer size big enough to hold this many samples
     **/
    private int determineCalculatedBufferSize(final int sampleRate, int encoding, int numSamplesInBuffer) {
        ...
        // each sample takes two bytes, need a bigger buffer if (encoding == AudioFormat.ENCODING_PCM_16BIT)
    ...
    private boolean doRecording(final int sampleRate, int encoding,
                                int recordingBufferSize, int readBufferSize, int bufferIncreaseFactor) {
        ...
        recorder = new AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate,
                        AudioFormat.CHANNEL_IN_MONO, encoding, increasedRecordingBufferSize);
        ...
        //if audio recording is already recording or otherwise not available //AudioRecord.getState() will be AudioRecord.STATE_UNINITIALIZED recorder.startRecording();
     ...
     **/
    private void setOnPositionUpdate(final short[] audioData, final int sampleRate, int numSamplesInBuffer) {
        AudioRecord.OnRecordPositionUpdateListener positionUpdater = new AudioRecord.OnRecordPositionUpdateListener() {
            @Override
            public void onPeriodicNotification(AudioRecord recorder) {
                // no need to read the audioData again since it was just // read
